...
function createPageHref(searchParams: NewsListParams, page: number) {
  ...
  query.set("limit", String(searchParams.limit ?? 5));
    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-10 rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-slate-100 p-8 shadow-sm">
        ...
        <h1 className="mt-3 text-3xl font-semibold text-slate-900 sm:text-4xl">
          Latest updates on global trade & policy
        </h1>
        <p className="mt-3 max-w-3xl text-slate-600">
          Read curated insights, regulatory changes, and practical market
          intelligence from our editorial team.
        </p>
      </div>
      ...
      <section className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
        {items.map((news, index) => (
          ...
          >
            {news.coverImage ? (
              <div
                className={`relative overflow-hidden ${index === 0 ? "h-72" : "h-52"}`}
              >
      ...
      {meta ? (
        ...
        >
          <Link
            href={
              previousPage ? createPageHref(searchParams, previousPage) : "#"
            }
