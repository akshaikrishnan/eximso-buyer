...
const EditAddressModal: React.FC<EditAddressModalProps> = ({ address, onClose, onSave }) => {
    const [formData, setFormData] = useState<Address>(address);
    const queryClient = useQueryClient();

    const updateAddressMutation = useMutation({
        mutationFn: async (updatedAddress: Address) => {
            const response = await api.put(`${endpoints.address}/${updatedAddress._id}`, updatedAddress);
            return response.data;
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["AddAddress"] });
            toast({ title: "Success", description: "Edited successfully!", variant: "default" });
            onSave();
        },
        onError: (error) => {
            console.error(error);
            toast({ title: "Error", description: "Failed to edit address!", variant: "default" });
        }
    });
