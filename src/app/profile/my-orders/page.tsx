...
const orders = [
  {
    ...
    status: "Delivered",
    total: "$50.00",
    image: "https://rukminim2.flixcart.com/image/850/1000/knqd3m80/support/e/3/w/na-toe-corrector-device-bunion-treatment-orthopedic-brace-splint-original-imaf4tzefsrnmzrb.jpeg?q=90&crop=false",
    color: "blue",
  ...
  },
  {
    ...
    status: "Cancelled",
    total: "$30.00",
    image: "https://m.media-amazon.com/images/I/51uuF4s+5iL._AC_UF1000,1000_QL80_.jpg",
    color: "blue",
...
export default function MyOrders() {
  return (
    <div className="container mx-auto py-8 max-w-4xl">
      <h1 className="text-2xl font-bold text-gray-800">My Orders ({orders.length})</h1>
      <div className="mt-6 space-y-6">
        {orders.map((order) => (
          <div key={order.id} className="flex items-center justify-between border-b border-gray-200 border shadow-xl pb-4">
            {/* Product Details */}
    ...
    whiteSpace: "normal"
                ...
                <p className="mt-1 text-sm font-normal ">Color: {order.color}</p>
              </div>
            </div>
            {/* Price and Status */}
            <div className="text-center p-4">
              <h1 className="text-lg font-semibold text-gray-900">{order.total}</h1>
              <p className="text-xs font-normal">Delivered on {order.date}</p>
              <p className="text-xs font-normal">Your item has been {order.status}</p>
            </div>
          </div>
        ))}
      </div>
